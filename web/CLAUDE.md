...
**2026-07-17 (latest) — Logged-in member payment decisions for events + bundles (same branch/PR #14). ONE additive migration added AND APPLIED via Supabase MCP (+bookkeeping, checksum `6ea53c75…`): `20260718010000_bundle_payment_decision` (`event_bundles.paymentMethods`; new `event_bundle_purchases` with a PARTIAL UNIQUE index on (bundleId, memberId) WHERE status NOT IN (CANCELED, PAYMENT_FAILED) — duplicate live purchases are structurally impossible; the index exists only in SQL, Prisma can't model it, don't "fix" the schema by removing it).**

1. **Members never get sent straight to Stripe anymore.** The member event register route used to default to CARD when multiple methods were allowed (the modal only appeared when CARD wasn't an option). Now a decision is required whenever >1 method is selectable; `PAYMENT_METHOD_REQUIRED` responses carry `options`, an exact `quote` {base, cardFee, cardTotal, offlineTotal}, `savedCard` label, and the event's `documents` (with per-athlete signed/ack status) so the portal modal shows everything before the member confirms.
2. **`SAVED_CARD` (pay now with card on file)** — offered for events AND bundles whenever CARD is allowed + a verified saved card exists. Events: creates the registration as SCHEDULED with `scheduledChargeAt=now` and runs the EXISTING `chargeEventRegistration` inline (zero new charge code — same idempotency/Transaction/receipt/audit); **the Booking is created ONLY after Stripe confirms** — a failed charge books nothing and marks nothing paid. Bundles: `chargeBundlePurchaseSavedCard` (lib/bundlePurchases.ts) with the same discipline (prior-PI settle before create, fail-closed on unverifiable, idempotency key `aox-bundlepur-<id>`). The client always sees the exact total on the confirm button.
3. **`lib/bundlePurchases.ts` is the bundle model.** `EventBundlePurchase` = a member's claim + the money owed. **Bookings (usable credits) are granted ONLY on PAID** — or at acceptance under the existing `clubs.offlineActivationPolicy` ON_ACCEPTANCE (no new setting). One DISTINCT status per method: AWAITING_CASH / AWAITING_CHECK / PAY_LATER ("club will invoice" — explicitly NOT a scheduled card charge) / PENDING_PAYMENT / PAYMENT_FAILED / PAID — never one vague pending. Cash/check/pay-later NEVER touch Stripe. `settleBundlePurchase` is the ONE settlement path (webhook `bundlePurchaseId` metadata, saved-card success, staff offline receipt) — conditional claim first, so replays/double-clicks settle once. Pay-later PENDING Transactions carry `paymentSource MANUAL_ADJUSTMENT + paymentMethod INVOICE` (that signature is how the offline-payment surfaces include them).
4. **`/api/members/[id]/offline-payment` POST now settles linked event registrations AND bundle purchases** via `transactionId` (previously staff recording event cash through the member card left the registration stuck AWAITING while the Transaction said paid). Owner enables bundle methods on the bundle editor (checkbox list); the member modal shows bundle documents grouped per included event, labeled "must be completed before <event> check-in" — **purchase-time signing is deliberately NOT required for bundles**; check-in stays the hard gate.
...
- **Migration folders were pre-dated for Phase 3.** The latest migration on disk is `20260801040000_marketing_audiences` (Phase 3), applied 2026-07-30. `npx prisma migrate deploy` sorts folders lexicographically and warns loudly on out-of-order migrations. **Any new migration folder MUST sort after `20260801040000`** — use `20260801050000` or a later timestamp. Do not backfill a folder with today's real date if it would sort before an already-applied one.
- **M16 `email_sends` — the double-send dedup is a PARTIAL unique index that Prisma cannot model.** The SQL migration defines `UNIQUE (sendBatchId, dedupeKey) WHERE sendBatchId IS NOT NULL AND dedupeKey IS NOT NULL`. Partial indexes are not expressible in Prisma's schema DSL, so `@@unique([sendBatchId, dedupeKey])` in `schema.prisma` is **documentation only** — the SQL migration is authoritative. Do not "reconcile" this drift in a future session (e.g. by adding a WHERE clause the schema can't emit, dropping the schema-side `@@unique`, or running `prisma db pull`). Adding a `.env` `SHADOW_DATABASE_URL` would also try to reconcile — leave that unset.
