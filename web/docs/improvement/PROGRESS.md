...
## 2026-09-14 — Financials deep links (B4: the four dead Action Item links)

Code-only, no migration. Branch `main`, main checkout. Julian runs the verify loop and pushes.

**What was wrong.** `/dashboard/financials` parsed no query parameters — `tab` was plain `useState` — so every card that linked in with `?tab=…` landed on Overview with nothing selected. The 2026-08-16 sweep had already downgraded three Action Items to bare `/dashboard/financials` links (UNRECONCILED_DEPOSIT, OFFLINE_PAYMENT_PENDING, UNCATEGORIZED_LARGE_BANK) and pointed UPCOMING_RENEWAL_LARGE at the bare roster; `lib/reportsAlerts` and the Reports Costs/Snapshot tabs were still emitting `?tab=bank`, `?tab=bank&filter=needs_review` and `?tab=out` into the void.

**Changes.**
- `app/dashboard/financials/page.tsx` — reads `?tab=` (validated against `TABS`) and a one-shot `?show=` focus at mount, mirrors the tab back into the URL on click via `router.replace` (same pattern as `/dashboard/reports`). `filter=needs_review` accepted as an alias of `show=review`. Three tabs gained the toggle the cards needed: Stripe "Unreconciled" (SUCCEEDED rows not `VERIFIED`), Cash & Offline "Awaiting receipt" (PENDING rows), Bank "Needs review only" (`bankRowStatus` Needs review / Suggested, client-side over the loaded page). Each has its own empty-state copy so a deep link that lands on nothing says so.
- `lib/reportsActionItems.ts` — UNRECONCILED_DEPOSIT → `?tab=stripe&show=unreconciled`; OFFLINE_PAYMENT_PENDING → `?tab=offline&show=awaiting`; UNCATEGORIZED_LARGE_BANK → `?tab=bank&show=review`. `lib/reportsAlerts.ts` UNCATEGORIZED_LARGE_BANK → `?tab=bank&show=review`. The Reports tabs' existing `?tab=out` / `?tab=bank` links now work without change.
- `scripts/reports-tests.ts` — dead-link guard: every `/dashboard/financials?` href in the Action Item and Alert modules must name a tab the page defines, a `show` the page parses, and nothing else; no bare financials link may remain.

**Not changed — Julian's call:** UPCOMING_RENEWAL_LARGE still opens the bare roster. There is no roster queue for "renewals ≥ $X in the next 7 days"; the nearest existing queue is `?queue=endingSoon` (120-day window, `lib/membersQuery.ENDING_SOON_WINDOW_DAYS`). Options: (a) point at `endingSoon` and accept the wider window, (b) add a `renewingSoon` queue keyed on `currentPeriodEnd/endDate` within 7 days, (c) leave bare.

**Verify:** `cd web && npx tsx scripts/reports-tests.ts && npx tsc --noEmit && npm run build`, then in the browser: `/dashboard/financials?tab=offline&show=awaiting` should open Cash & Offline with "Awaiting receipt" lit. Non-incremental `tsc --noEmit` clean from the sandbox.

## 2026-09-14 — Branch review: `claude/elated-noether-46e7d6` is superseded (B8)

Read-only review, no code change. Five commits, 2026-08-21/22, merge-base `71933a3`, local only (no `origin/` ref), checked out in worktree `web/.claude/worktrees/nifty-pasteur-1ecb47` (listed "prunable").

**Three of the five are already on main by content** — `git cherry` marks them `-` (equivalent patch applied): `7fdc8a8` Stop reporting a send timeout as a clean failure, `38da296` Write down how to kill a stray dev server, `6c7bf6b` Instrument the bulk send. `web/app/api/members/bulk/route.ts`, `lib/phaseTimer.ts`, `scripts/seed-bulk-send-scale.ts` and `MemberModals.tsx` are byte-identical between the branch and main, and CLAUDE.md on main carries the dev-server section (plus 96 more lines the branch never saw).

**The other two are a parallel implementation of work main did separately** — the exact "one branch per session" failure mode CLAUDE.md warns about. `5dfdca9` Add the coverage resolver (2026-08-21) and `6c07b81` Wire the coverage verdict into the staff attendance paths duplicate main's `8ece3b1` Add the coverage resolver (2026-08-18 — three days EARLIER) and `b2b94e0` Show staff, as they add someone, that the day is not in their plan (2026-08-22). Main's version went further: `lib/coverageQuery.ts` is the loader (the branch's `lib/entitlementsServer.ts` is its twin), the attendance page has the QuickAdd chip the branch commit lists as "still to come", `lib/entitlements.ts` is 291 lines vs the branch's 212, the tests 56 assertions vs 43, and the class charge route already diverts on `DAY_NOT_INCLUDED` (lines 78/148). The branch's one distinct idea — prefer the subscription on an accepted plan when a member holds two — is moot because main's loader passes every subscription to the pure resolver rather than picking one. Main's `POST /api/attendance` does not echo a verdict on the response, which the branch did; the chip-before-add on main makes that redundant, and the 2026-09-13 attendance-billing gate covers the no-membership case.

**Verdict: nothing to merge, nothing to cherry-pick. Delete the branch and prune its worktree.** Julian runs (from `~/Desktop/clubos`): `git worktree remove --force web/.claude/worktrees/nifty-pasteur-1ecb47 && git branch -D claude/elated-noether-46e7d6 && git worktree prune`. The seven other stale worktrees are also "prunable" and belong to branches already merged or reviewed; `git worktree prune` alone clears the registrations once their directories are gone, but that cleanup is a separate call.

## 2026-09-22 — Scoping the two design handoffs (read-only, no code)

Both handoffs are now in git (`658d28c`): `docs/improvement/design_handoff_event_editor/` and `docs/improvement/design_handoff_products/`. Each README is the spec; the `.dc.html` prototypes are the behaviour reference, not code to port.

### Products handoff — can start now

**Exists today:** `app/dashboard/products/page.tsx` (763 lines: table, `ProductModal`, `SellModal`), `app/member/products/page.tsx` (193, list only), `app/member/shop/page.tsx`, `api/products` + `api/products/[id]` + `api/products/[id]/sell`, `api/member/products` + `api/member/products/[id]/buy`, `components/StaffDiscountPicker.tsx`, `components/QRModal.tsx`. `Product` model: one `imageUrl`, `visibility` + `showLocation`, `trackInventory` + a single `inventory` int, `productType` string (GEAR | FACILITY_RENTAL | BIRTHDAY_PARTY | DIGITAL | OTHER), and a `settings` JSON that holds everything type-specific as free text (`variantOptions`, `variantStock`, `packageTiers`, `durationPrices`, `timeWindows`, `addOns`, `customQuestions`, `blackoutDates`, `maxGuests`, `depositMode`, `depositAmount`). `ProductSale` has no variant column. No `/p/[slug]` public route, no product-booking model, no scan counting.

**Design asks for:** 11 product types; up to 4 photos; a real variant matrix (stock/price/SKU/photo per Size×Color); structured tiers, durations, time windows, add-ons, questions; `storefronts[]` replacing `visibility`+`showLocation`; public `/p/{slug}` page + QR sheet; an inventory screen (2c); a rental/party booking calendar + member booking flow (2d/2f — needs a NEW `ProductBooking` model); Sell by variant (2g); member store detail with per-variant stock (2e).

**Files it touches:** the four pages above, `components/member/ItemCard.tsx`, the five product API routes, `prisma/schema.prisma` (slice-dependent), new `app/p/[slug]`, new booking model + routes. Private lessons explicitly out of scope.

**Conflicts:** COPPA branch adds a 6-line consent gate at the top of `api/member/products/[id]/buy/route.ts` — the only shared file; trivial to re-apply, and it is not in slice 1. Phase 9 does not touch products at all (memberships-only at launch; `DiscountItemType.PRODUCT` exists but `appliesTo` is validated to `["MEMBERSHIP"]`). The event-pricing session touches nothing here.

**Smallest first slice (no migration):** 2a + 2b. Structured editors writing into the existing `settings` JSON as typed shapes (`optionGroups[]`, `variants[]`, `tiers[]`, `durations[]`, `addOns[]`, `questions[]`, `photos[]`), one parser that reads the old free-text shapes into them (the README's migration rules), and the product-list cards reading the same variant ledger for stock badges and `n low / n sold out` chips. Keep `visibility`/`showLocation` as the stored columns with `storefronts` derived both ways. That gives Julian the "how many Youth M are left" answer with zero schema risk. Slice 2: 2g Sell by variant + 2e store detail (adds `variantId` to `ProductSale` — one additive migration — and touches the COPPA-gated buy route, so merge COPPA first). Slice 3: 2c inventory. Slice 4: 2d/2f bookings (new model). Slice 5: `/p/[slug]` + QR sheet.

### Event editor + Attendees handoff — WAIT

**Exists today:** `app/dashboard/events/page.tsx` (4,614 lines: `EventModal` @689, `BookingsModal` @2530, `RegistrationsModal` @3319, list card, `REG_STATUS_UI`), `api/events/[id]/{bookings,registrations,charge,bill-registrants,reprice-registrations,…}`, `api/member/events` + `api/member/events/[id]/register`, public `/e/[slug]`. `Event` already has most of the storage the design wants (`memberPrice/nonMemberPrice/dropInFee`, `variableCost*`, `invoiceScheduledAt`, `paymentMethods`, `autoChargeDate`, `requirePaymentBeforeCheckin`, `requiresCoachApproval/approvalPaymentIntent/responsibleCoachUserId`, `capacity`, `paymentDueBy`, `cancellationPolicyText`, `registrationForm`, `visibility/purchaseAccess/publicRegistration/publicPricingOption`). `EventSession` has NO price.

**Design asks for:** a one-scroll collapsible editor (phone + desktop) with exclusion rules; `pricingModel` FREE|FIXED|SPLIT replacing the implicit combination; per-session prices (`EventSession.price`) replacing `dropInFee`; `signupAccess` replacing three controls; `splitInvoiceWhen`; participant category on every type; one Attendees list merging `Booking` + `EventRegistration` (one LIST — the tables stay separate per the event money model; adding creates registration then booking, removing cancels the registration which cascades); three event-row treatments surfacing outstanding money.

**Files it touches:** `events/page.tsx` (most of it), `api/events/route.ts` + `[id]/route.ts` (new fields, migration for `EventSession.price` + `pricingModel` + `signupAccess` + `splitInvoiceWhen`), `[id]/bookings` + `[id]/registrations` (or a new merged attendees endpoint), **`api/member/events/route.ts` + `[id]/register/route.ts`** (the `DROP_IN` pricing branch goes away with `dropInFee`; `isActiveMember` gating), `/e/[slug]` + `api/public/events/[slug]`, `lib/eventCategories.ts`.

**Conflicts:** (1) the event-pricing session — B1 shipped `7efbe0e` on 2026-09-14 changing exactly `api/member/events/route.ts` and `[id]/register/route.ts`; if another session is still in those files, this handoff must not touch them until it merges. (2) COPPA adds a 6-line gate to `[id]/register/route.ts` — same file, different region; merge COPPA before the register-route rewrite. (3) Phase 9: no file overlap; it cites `Event.registrationForm` and `lib/eventCategories.ts` only as the precedent to copy. (4) `CLAUDE.md` event money model: registration is the money spine, Booking is derived — the merged Attendees list must be built on that, not on a new table.

**Smallest first slice when unblocked (no migration):** 1e event-row treatments + 1c/1d read-only Attendees list — a single `GET /api/events/[id]/attendees` that joins bookings and registrations into the README's row shape, driving the tiles, filter chips and capacity bar. No editor changes, no schema, doesn't touch the member pricing routes. The editor rewrite (1a/1b) is slice 2 and carries the migration.

### Order

Products first (slice 1 is self-contained and unblocked). Events after the pricing session merges and, ideally, after COPPA (B2) lands — which is itself waiting on A3.

## 2026-09-22 — Events handoff, slice 1: event rows (1e) + read-only Attendees (1c/1d)

Code-only, NO schema, NO migration, NO editor changes. Built on `main` at `658d28c`; Julian moves it onto a branch, runs the build and commits — it is not merged. The member event list and register routes (`api/member/events*`) were **not touched** (the COPPA branch edits the register route and merges later).

**The rule this keeps.** EventRegistration is the money spine; Booking is derived. This slice reads both and writes neither. The Attendees screen is one LIST for display — the tables stay separate — and every write action (record cash/check, approve/decline/propose, resend, add a member) still goes through the Registrations and Bookings modals, which the new screen links to. The new endpoint deliberately does NOT run the lazy charge/reminder sweep the registrations GET runs: a list people open to look at must not be the thing that charges a card.

**New.**
- `lib/eventAttendees.ts` — PURE ledger. `buildAttendeeLedger(event, registrations, bookings)` joins on memberId (registration wins status + money; booking adds waitlist state; unmatched public registrations and booking-only members are their own rows), derives `owes` (UNPAID statuses only: `amountDue − amountPaid`), tiles (collected / outstanding / scheduled / waiting on you), filter counts, capacity segments (settled / owe / scheduled / review) and `spotsLeft`. `summarizeLedger()` is the per-event summary the list rows use. `ATTENDEE_STATUS_UI` = `REG_STATUS_UI` + `COVERED` (booking-only on a free or membership-covered event) + `WAITLISTED`.
- `lib/eventAttendeesServer.ts` — the ONE loader. `loadEventAttendees(clubId, eventId)` (recipients resolved through the family model, category label from `resolveCategoryFields`) and `loadEventMoneySummaries(clubId, events)` (two queries for the whole list, never one per event).
- `GET /api/events/[id]/attendees` — read-only, `events:view`.
- `GET /api/events` now attaches `money: EventMoneySummary | null` to every event (older callers unaffected).
- `components/events/EventRow.tsx` — the three treatments: money-first row (date pill, type chip, "$x to collect" warn chip, meta line, segmented capacity bar + counts line, `Attendees · n` / Edit / ⋯), compact row, cover-photo card (cover from `Event.imageUrl`, striped placeholder otherwise, pill row). All three read `Event.money`.
- `components/events/AttendeesModal.tsx` — header (`Attendees · n`, event · dates · spots left · /e/slug), explainer, four tiles, live filter chips (All / Owes money / Waiting on you / Card scheduled / Settled), desktop table with one shared track list (Name+source chip / Contact = family-resolved recipient / {category label} / Attending / Owes / Status pill), phone cards (initials avatar, amount, pill), footer with "Show n removed" and the outstanding-of-total line. "Record / approve" and "+ Add attendee" open the existing modals.
- `app/dashboard/events/page.tsx` — a Money / Compact / Cards toggle beside the Upcoming/Past/All chips (remembered per browser in localStorage), rows rendered by `EventRow`, the old desktop button row folded into the ⋯ sheet (now also on desktop, centred), an Attendees entry added to it, and the `AttendeesModal` mounted. The old scoreboard card is gone; `EventModal`, `BookingsModal`, `RegistrationsModal` are untouched.
- `scripts/event-attendees-tests.ts` (`npm run test:event-attendees`) — 40 assertions: join rules, money per status (abandoned checkout owes nothing, partial payment owes the remainder, SCHEDULED owes nothing yet), tiles = filters = capacity segments from one row set, covered-vs-registered booking-only rows, category and family-resolved contact.

**Not in this slice (slice 2, needs a migration):** per-session prices, `pricingModel`, `signupAccess`, `splitInvoiceWhen`, the collapsible editor, inline record/approve actions on the Attendees rows, Export, check-in mode, "Attending: session subset".

**Verify:** `cd web && npm run test:event-attendees && npx tsc --noEmit && npm run build`. Non-incremental `tsc --noEmit` clean from the sandbox (tsx/build must run on the Mac).

## 2026-09-22 — Products handoff, slice 1: structured editor (2a) + product cards (2b)

Code-only, NO schema, NO migration, NO API route changes (the products routes already accept `settings` as an open JSON record). Built on `main` at `0c38253`; Julian moves it onto `claude/products-slice-1`, runs the build, commits, pushes. Not merged. The member buy route (`api/member/products/[id]/buy`) is untouched.

**What was wrong.** Every type-specific product field was a "one per line" textarea stored as `string[]` in `Product.settings` — `variantOptions`, `variantStock`, `packageTiers`, `durationPrices`, `addOns`, `customQuestions` — and the stock notes' own hint admitted they were "simple stock notes for staff until variant checkout is connected". So the app could not answer "how many Youth M are left". One photo per product; `visibility` + `showLocation` overlapped and could contradict.

**New.**
- `lib/productSettings.ts` — PURE. The typed v2 shape (`optionGroups[]`, `variants[]` with stock/price/SKU/photo per row, `tiers[]`, `durations[]`, `addOns[]` with per-guest, `questions[]` with kind/required, `photos[]` ×4, deposit mode, days, blackout dates…), versioned `v: 2` in the SAME `settings` column. `normalizeProductSettings()` is the one parser: upgrades a v1 blob with the handoff's migration rules (`"Basic: 60 minutes, $150"` → tier row; `"Youth S / Black: 8"` → variant row; `"30 minutes: $40"` → duration; `"Pizza (per guest): $12"` → per-guest add-on), idempotent on v2. `reconcileVariants()` rebuilds the Size×Color matrix when groups change while KEEPING the stock/SKU/price of every row that survives — and keeps any orphaned row that still holds units (counted stock is never silently lost; a 0-stock orphan drops). `variantLedger()` is the one source for units / retail value / low / sold-out; `derivedInventory()` writes `Product.inventory` as the variant total so the sell and buy routes' stock checks keep working unchanged. `storefrontsFor()` / `columnsForStorefronts()` are the only interpreters of the `visibility` + `showLocation` pair, in both directions (public link wins over members-only; nothing ticked → staff-only, never silently public).
- `components/products/ProductEditor.tsx` — screen 2a: type chips with the per-type stock note; collapsible cards whose collapsed summary states the current answer (Basics & photos ×4 with COVER badge; Pricing with member price + "Sell this as tiers" switch and tier table + derived range line; Inventory & variants with option-group chip editors, the variant matrix (SKU/price/stock/status pill, red border at 0, amber at threshold), "Set all stock…", "Apply base price to all", derived units/retail line, locked with the type's reason when the type holds no stock; Booking & availability locked for non-bookable types, with days, time windows, buffer/per-slot/max guests, Length & price table, three exclusive payment-mode cards where the deposit amount exists only in Deposit mode, add-ons table with Flat/Per guest, staff-approval switch, blackout chips; Questions at checkout for every type (+ digital delivery for DIGITAL); Where it's sold as three checkboxes ↔ the two columns, fulfillment greyed with a reason for types with nothing to hand over; a brand-tinted recap panel stating the consequence of the settings). The 5 existing `productType` values are kept — the handoff's 11-type expansion changes the enum the API validates and is a later slice. Private lessons stay out.
- `components/products/ProductCard.tsx` — screen 2b: photo-card with stock badge (`30 in stock` / `Out of stock` / `Bookable` / `Unlimited` / `Inactive`), type line, price or tier range, sold count, derived chips (`1 sold out`, `4 low`, `6 variants`, `3 tiers`, `Hidden from store`), primary action (Sell / Restock / Bookings / Activate) and ⋯. `deriveCard()` is the one derivation the cards, filter counts and page subtitle share.
- `app/dashboard/products/page.tsx` — derived subtitle (`n products · $x inventory at retail · n need attention`), filter chips with live counts (All / Gear / Bookable / Low or out / Inactive) + search, 4-column card grid replacing the table, `ProductEditor` replacing `ProductModal`. `SellModal` untouched (still sells by product, not variant — slice 2).
- `scripts/product-settings-tests.ts` (`npm run test:product-settings`) — 51 assertions: every line parser, the v1→v2 upgrade incl. orphan stock notes, idempotence, the matrix rebuild rules, the ledger, `derivedInventory`, tier range, and the storefront mapping both ways. Found one real bug before shipping: re-normalising a v2 blob dropped orphaned stock rows.

**Not in this slice:** Sell by variant + store detail with per-variant stock (2g/2e — one additive migration, `ProductSale.variantId`; merge nothing first, COPPA is already on main), inventory screen (2c), bookings calendar + member flow (2d/2f — new model), `/p/[slug]` + QR sheet (2h), the 11-type expansion, structured time windows.

**Verify:** `cd web && npm run test:product-settings && npx tsc --noEmit && npm run build`. Non-incremental `tsc --noEmit` clean and the 51 tests pass (transpiled and run in the sandbox); `next build` must run on the Mac.

## 2026-09-22 — B9 diagnosis: "I changed the membership and nothing happened" (read-only, no code)

Reproduced on two members from Julian's screenshots + production reads. **Verdict: not a crash — a product gap. There is no owner-side action that turns what the billing centre saves into a subscription, and no owner-side action that changes a Stripe-billed member's plan at all.** Every screen Julian used did exactly what it was built to do; what it was built to do is not what an owner needs.

**What the billing centre "Edit" writes.** `PATCH /api/members/[id]/billing-admin` writes ONLY the migration draft on the `Member` row — `migrationMembershipId`, `migrationSelectedOption`, `migrationPriceOverride`, `migrationFinalPeriodPaid`, `billingAnchorDate`, `commitmentEndDate`, `requestedPaymentMethod`… It never creates or updates a `MemberSubscription` and never touches Stripe. The page says so ("Saving changes here does not charge the client. They take effect only when the client confirms the reactivation offer or an authorized user explicitly activates the membership") — but the second half is not true for an owner: the only activation paths are the client confirming a reactivation offer, or the client buying in the portal. "Explicitly activates" has no button behind it.

**Colton Waite (production).** `Member.status = INACTIVE`. His only subscription is the imported MANUAL $530 MONTHLY row, status **expired** — the lazy `expireEndedManualSubscriptions` sweep flipped it when `endDate` 2026-09-08 passed. On Sep 11 Julian saved the draft: MS/HS · 3 months Upfront $450 QUARTERLY, anchor Sep 11, commitment Dec 10, "Final period already paid". Nothing read that draft since. Readiness shows "Already active / leave alone" because `migrationStatus = COMPLETED` short-circuits `deriveReadiness` before the "final period paid → READY" rule. Net: a former member, training, with no active row — exactly the unbilled-athlete shape.

**Orson Chorba (production).** `Member.status = ACTIVE`, live Stripe subscription `sub_1TsknzE…` on MS/HS · Monthly $175, `stripeStatus = trialing`, autoRenew on, next real charge on the Stripe anchor (Sep 24). On Sep 21 Julian saved the draft as 12 months · $150 MONTHLY. The Edit modal re-opens showing 12 months (it reads the draft) while the summary card shows Monthly $175 (it reads the live sub, which `deriveBillingState` ranks above DRAFT_CONFIG). "Preview changes → No changes" is the modal comparing the draft to itself. The live Stripe subscription is unchanged and will bill $175 on Sep 24. The two owner paths that COULD change a live plan both refuse Stripe rows on purpose: the bulk price tool (`canChangeOption: !isStripe` — "move this one from the billing centre instead") and the reactivation offer (`409 … already has a live Stripe subscription`). The billing centre has nothing to move it with. Circular.

**Also seen:** Orson's "Owner-approved final billing date Jul 24, 2026" / "Next billing Jul 24, 2026" is a stale `requestedBillingDate` (2026-07-03 import) rendered as if it were the next charge — the member-level-field-as-current-answer bug class from CLAUDE.md. `currentPeriodEnd` is null on his row because the Stripe reconciler hasn't run for him; when it has, that line should read Stripe's date, never the migration field.

**Where each owner action actually lives today** (this is the design problem): assign a membership = roster row menu → Purchase membership (`/api/members/subscribe`; not on the profile, not in the billing centre); edit dates/price/status of an existing row = profile → Current membership → Edit (`PATCH /api/members/subscriptions/[id]`, local only — a Stripe row keeps billing its own amount); move offline rows between options = Memberships → bulk price change (Stripe rows refused); change a Stripe row's plan = nowhere; cancel = the profile Edit's status dropdown set to "canceled" (`DELETE` on the same route cancels Stripe too, but no button calls it); set up a future/offline activation = billing centre Edit + Create offer → the CLIENT confirms. Four screens, five routes, and the one the owner reaches for first (the billing centre) is the only one that writes nothing real.

**B9 = build the missing owner action, in the billing centre:** "Activate this setup now" for members with NO live Stripe row — creates the MemberSubscription from the draft (plan/option/price/period/start/term/autoRenew/payment method) through the same code `/api/members/subscribe` uses, marks the previous active/expired MANUAL row superseded, writes `BillingAuditLog`, recomputes status; with "Final period already paid" it is created active with `paidThroughDate` = commitment end and no pending Transaction, otherwise as pending-cash/check per `offlineActivationPolicy` with the PENDING Transaction the offline-payment route expects. Plus: "Assign membership" on the profile's Current membership card when there is none (opens the existing PurchaseMembershipModal), and the readiness fix so COMPLETED + final-paid reads READY, not "leave alone". No migration. Colton and Wyatt both go through this. **Stripe plan change is NOT B9** — see B12.
