...
**2026-07-13 (final, pre-merge) — go-live state of PR #6 (9 commits, tip `439764e`). Read this block first when resuming.**

- **At-the-door "Card" relabel (`439764e`, copy-only)**: the attendance drop-in/pay forms' CREDIT option now reads **"Card (external)"**, buttons say "Record externally-collected card payment" / "Record external card … & mark Drop-in", with an orange hint that AthletixOS does NOT charge the card. RULE going forward: **every surface that says "Card" must either really charge through Stripe (checkout/subscription with receipt + Transaction) or state record-only in the staff's face.** The at-the-door path records only — a true tap-to-pay capture is future work.
- **Owner-instructed Titus Hall ops (data, via Supabase MCP + Stripe API — not code)**: his 2 still-open abandoned $1,500 checkout sessions were EXPIRED in Stripe (customer verified clean: no subs/invoices/charges/cards), the 3 pending sub rows canceled with notes, and reactivation offer **v1 DRAFT staged but NOT sent** — MS/HS "1 Year" ($2,000 option, $1,500 owner override), start + first charge 2026-07-20, commitment through 2027-07-20, payer Shannan Hall, card collected at confirmation. Waiting on Julian's explicit "send".
- **Go-live verdicts (2026-07-13 audit)**: placeholder creation is blocked (NOTHING_CONFIGURED guard) and cleanup can wait; self-serve purchase is complete ON THIS BRANCH (customer capture + Financials fixes) but has never been live-fire tested with a real card — watch the first purchase after merge; **client change-requests on the reactivation page are NOT BUILT** (deliberate post-migration scope — parents call/text, owner regenerates); at-the-door was the only false-"Card" surface and is now labeled honest. Other known non-blockers: single private lessons have NO payment path (bookings sit unpaid unless staff record money manually), product-sale webhook writes no Transaction row, signup wizard still uses typed acknowledgement, member-portal purchases never reuse the saved card (fresh Checkout each time).
- **Pending owner actions after merge** (all staged, none executed): watched $1 live purchase; "send Titus's offer"; `fix-placeholder-memberships.ts` dry-run→apply for Harrison Bossert, Oliver Stacconi, Colton Clark (2 subs), Colton Graham, Parker Strickland (Jackson Dolson excluded — reactivation planned); John Doe demo teardown (cancel the $5 test Stripe sub — its first Transaction is also missing from Financials because it predates the invoice.paid race fix); Group B reactivation offers (Demir, Levi, Max Hall — Orson already confirmed live: trialing, first charge 2026-07-24, his MANUAL placeholder auto-superseded). Lazy expiry sweep will flip James Bossert, Julien Bossert, Avery Mosher, Zach Boudreau to INACTIVE on the first members-list load after merge (ended final-paid periods).
