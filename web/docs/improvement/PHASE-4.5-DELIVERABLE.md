...
| D-1b | Data correction | `scripts/fix-guardian-contact-bleed.ts` — dry-run default, minors only, never touches a member with their own login, guardian columns untouched, every write logged with the old value. **Yours to run.** |
...
```bash
cd /Users/cubano/Desktop/clubos/web/.claude/worktrees/nifty-pasteur-1ecb47/web && npx tsx scripts/members-experience-backfill.ts
```
...
```bash
cd /Users/cubano/Desktop/clubos/web/.claude/worktrees/nifty-pasteur-1ecb47/web && npx tsx scripts/members-experience-backfill.ts --apply --clubs=<clubId>
```
...
```bash
cd /Users/cubano/Desktop/clubos/web/.claude/worktrees/nifty-pasteur-1ecb47/web && npx tsx scripts/fix-guardian-contact-bleed.ts
```
