...
// are allowed to differ — session D confirmed a live case where a member's
// contact was julianramirez1181@gmail.com while their login was
// hello@athletix-os.com. PATCH /api/members/[id] writes the CONTACT column and
// deliberately never touches a login; its only `prisma.user` reference is a
// read. That separation is correct and stays.
//
// What was missing is any way to reconcile the two on purpose. Staff could see
// the mismatch on Account & security and had no control to fix it, so the only
// options were to live with it or to guess that editing the contact email would
// eventually move the login (it does not).
//
// ── Guardrails ──────────────────────────────────────────────────────────────
//
