...
5. **There is still NO cron.** Due charges run via (a) lazy sweep when the registrations roster loads (`runDueEventCharges`, swallows its own errors) and (b) `POST/GET /api/cron/event-charges` gated on `CRON_SECRET` (**503 when unset — never open by default**) for an external scheduler. Both are idempotent. **Set `CRON_SECRET` in Netlify + point a scheduler at it hourly** for timely charges; without it charges still run whenever staff open the roster.
