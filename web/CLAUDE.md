...
Flow:
1. Owner **CSV-imports** members (`/dashboard/members/migration` — name is the only required field; for minors only guardian **name + email** are required (not phone); date-format picker, second-pass membership matching). Imported members carry a legacy snapshot, start `migrationStatus=IMPORTED`, and **always `status=PROSPECT`** — import never auto-assigns ACTIVE (a member only becomes ACTIVE after activation/approval). `normalizeStatus` maps everything except explicit inactive/paused to PROSPECT; the membership second-pass (`/api/members/import/memberships`) also downgrades a stale ACTIVE→PROSPECT when it pulls a member in (unless they have a live sub / already activated). The importer (`/api/members/import`) prefetches existing emails (one query), resolves each unique guardian once, and **creates rows in concurrent batches of 5** (`IMPORT_CONCURRENCY`, + `export const maxDuration = 60`) so a large or cold import can't hit the serverless function timeout — the old strictly-sequential per-row loop did, surfacing as a generic "Import failed" after a ~10s spin.
