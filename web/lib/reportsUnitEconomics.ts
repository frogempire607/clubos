...
export type UnitEconInputs = {
  revenue: number;
  expenses: Array<{ amount: unknown; category: string | null; kind: string | null }>;
  payroll: number;
  athleteCount: number;
  newMembers: number;
  marketingSpend: number;
  rangeMs: number; // range length in ms; used to convert fixed costs to a monthly figure
};

// Pure calculation of the UnitEconomics payload from ready inputs. Extracted
// from buildUnitEconomics for unit-testable break-even + CAC math.
export function computeUnitEconomicsCore(inp: UnitEconInputs): {
  perAthlete: UnitEconomicsPayload["perAthlete"];
  margins: UnitEconomicsPayload["margins"];
  breakEven: UnitEconomicsPayload["breakEven"];
  acquisition: UnitEconomicsPayload["acquisition"];
} {
  const { revenue, expenses, payroll, athleteCount, newMembers, marketingSpend, rangeMs } = inp;
  const totalExpenses = expenses.reduce((s, e) => s + Number(e.amount), 0) + payroll;
  let fixedCosts = payroll;
  let variableCosts = 0;
  ...
  const perAthlete: UnitEconomicsPayload["perAthlete"] = athleteCount > 0
    ...
    : { revenue: null, cost: null, grossProfit: null, operatingProfit: null, marginPercent: null };
  const margins = {
  ...
  return {
    ...
    margins,
    breakEven,
    acquisition: { cac, ltv, ltvToCacRatio, isEstimate: true, caveats },
  };
}
