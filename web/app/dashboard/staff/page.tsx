  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-semibold text-stone-900 mb-1">Staff</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-stone-200 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-stone-900 mb-1">No staff yet</h3>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      ...
                      {s.staffProfile?.title && (
                        <span className="text-xs text-stone-500">· {s.staffProfile.title}</span>
                      )}
                      {s.staffProfile?.hourlyRate && (
                        <span className="text-xs text-stone-400">${s.staffProfile.hourlyRate}/hr</span>
                      )}
                      {s.staffProfile?.salary && (
                        <span className="text-xs text-stone-400">${s.staffProfile.salary}/yr salary</span>
                      )}
                    </div>
                    <p className="text-xs text-stone-500 mb-2">{s.email}</p>
                    <div className="flex flex-wrap gap-1">
                      {activePerms.length === 0 ? (
                        <span className="text-xs text-stone-400">No permissions set</span>
                      ) : (
                        activePerms.map((p) => {
...
function PermissionRow({
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium text-stone-900">{def.label}</div>
        <div className="text-xs text-stone-500">{def.desc}</div>
      </div>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value as PermissionLevel)}
        className="text-xs px-2 py-1.5 border border-stone-300 rounded-md bg-white text-stone-900 focus:outline-none focus:ring-2 focus:ring-stone-900"
      >
        {def.levels.map((lvl) => (
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-stone-200 flex items-center justify-between sticky top-0 bg-white">
          <h2 className="text-lg font-semibold text-stone-900">Add staff member</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="pt-2 border-t border-stone-100">
            <p className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-3">Permissions</p>
            <div className="space-y-3">
      ...
      <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Annual salary ($)</label>
              <input type="number" min="0" step="0.01" value={salary}
                onChange={(e) => setSalary(e.target.value)} placeholder="0.00"
                className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-stone-900" />
            </div>
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Private rate ($)</label>
