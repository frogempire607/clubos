...
//
//  SELF_GUARDIAN must be finished before ORPHAN_MINORS runs. A conflated
//  account's `guardianEmail` points at itself, so the orphan sweep would look
//  at it, find a "live User" with a matching address, and re-create the exact
//  self-guardian link the split just removed — silently undoing the repair.
//
//  So ORPHAN_MINORS --apply COUNTS the remaining shape-A members first and
//  exits non-zero if any are left. It is a live-data check, not a flag the
//  caller can forget or lie about.
...
const MODES = ["SELF_GUARDIAN", "CHILD_EMAIL", "ORPHAN_MINORS", "AJ_DUPLICATE"] as const;
type Mode = (typeof MODES)[number];

// The canonical sequence. SELF_GUARDIAN leads because ORPHAN_MINORS is unsafe
// until it is done; AJ_DUPLICATE is last because it is a hand-off, not a write.
const ORDER: Mode[] = ["SELF_GUARDIAN", "CHILD_EMAIL", "ORPHAN_MINORS", "AJ_DUPLICATE"];
...
type Action = SelfGuardian | ChildEmail | OrphanMinor | AjDuplicate;
...
async function assertSelfGuardiansResolved(where: { clubId?: string }): Promise<number> {
  const remaining = await collectSelfGuardian(where);
  if (remaining.length === 0) return 0;
  console.error(
    `\nREFUSING to apply ORPHAN_MINORS: ${remaining.length} self-guardian member(s) still unresolved.\n\n` +
      remaining.map((r) => `  - ${r.name} (${r.memberId}) — ${r.userEmail}`).join("\n") +
      `\n\nA conflated account's guardianEmail points at itself, so this sweep would re-link it to itself and\n` +
      `undo the split. Run SELF_GUARDIAN for each of the above first:\n\n` +
...
async function main() {
  ...
  for (const mode of selected) {
    ...
    if (mode === "ORPHAN_MINORS") {
      const blocked = await collectSelfGuardian(where);
      if (blocked.length > 0) {
        console.log(
          `\n  BLOCKED: ${blocked.length} self-guardian member(s) remain, so --apply will refuse for this mode.\n` +
            `  Finish SELF_GUARDIAN first — otherwise this sweep re-links a conflated account to itself.`,
        );
      }
    }
